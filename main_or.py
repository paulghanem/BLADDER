...
# os.environ['KMP_DUPLICATE _ LIB_OK']='True'
...
#     parser.add_argument("qnoise", type=float, default=1.0)
#     parser.add_argument("experiment", type=int, default=0)
#     parser.add_argument("seqlength", type=int, default=1000)
#     parser.add_argument("compressionratio", type=int, default=5)
#     args = parser.parse_args()
    ...
    step_size = np.linspace(0,300*60,20000)[1]
    
    solution = np.load("ode_solution_20000.npy")[0:,:]
    solution[:,0] *= 1000
    # solution[:,0] *= 1000
    # solution[:,1] *= 1000
    #Initial conditions
    ...
    if integrator_type == "lsoda":
        lut.integrator.lsoda = scipy.integrate.LSODA(lut.forward_lsoda, t0 = 0, t_bound=300*60, y0 = lut.dictionary_to_array(initial_condition), max_step = 0.5)
    # """
        steps = 0
        outputs = []
        ts = []
        while not (lut.integrator.lsoda.status =="failed" or lut.integrator.lsoda.status =="finished") :
            steps += 1
            lut.integrator.lsoda.step()
            outputs.append(lut.integrator.lsoda.y)
            ts.append(lut.integrator.lsoda.t)
            if lsoda.direction == -1:
                print(steps, lut.integrator.lsoda.status)
        sol = np.asarray(outputs)
        ts = np.asarray(ts)
        for i in range(3):
            fig, ax = plt.subplots(figsize = (10,4))
            linewidth = 2
            ax.plot(ts, sol[:,i], linewidth=linewidth, markevery=5000)
            # plt.title("timesteps")
            fig.legend(ncol=3,loc='lower center', bbox_to_anchor=(0.5, -0.3))
            plt.savefig('lsodaoutputs_{}.png'.format(i), bbox_inches='tight', dpi=500) 
            plt.show()
    
        fig, ax = plt.subplots(figsize = (10,4))
        linewidth = 2
        ax.plot(np.diff(ts), linewidth=linewidth, markevery=5000)
        plt.title("timesteps")
        fig.legend(ncol=3,loc='lower center', bbox_to_anchor=(0.5, -0.3))
        plt.savefig('timesteps.png', bbox_inches='tight', dpi=500) 
        plt.show()
    # """
...
# def run_model(rnn, nncbf, N, ydata, mode='train', print_progress = True):
    ...
    for t in range(iterations):
        ...
        if ((t+1) % 5000 )== 0:
            ...
            fig, ax = plt.subplots(figsize = (10,4))
            linewidth = 2
            ax.plot(lut.physiological_component_dict["Bladder"].ode_component_dict["Bladder Afferent"].NBe_outs, linewidth=linewidth, markevery=5000,color='red')
            ax.plot(lut.physiological_component_dict["Bladder"].ode_component_dict["Bladder Afferent"].NBe_nn_outs, linewidth=linewidth, markevery=5000,color='blue')
            plt.title("NBe")
            fig.legend(ncol=3,loc='lower center', bbox_to_anchor=(0.5, -0.3))
            # plt.savefig('timesteps.png', bbox_inches='tight', dpi=500) 
            plt.show()
...
##        plot_freq = 100
##        if ((t+1) % plot_freq) == 0 or t==iterations-1:
##            if print_progress:
...
##            ax2 = ax.twinx()
##            linewidth = 2
##            ax.plot(ydata[0:t, 0], label=r'$V_{Bladder}$ - Ground Truth', marker=markers[0], linewidth=linewidth, markevery=5000, color='yellowgreen', markersize = markersize)
...
##            ax2.plot(ydata[0:t, 1]*5, label=r'$\phi_B$*5 - Ground Truth', marker=markers[0], linewidth=linewidth, markevery=5000, color='orange', markersize = markersize)
##            ax2.plot(unknown_state[0:t,1]*5, ':',linewidth=linewidth, label=r'$\hat{\phi}_{B}$*5 - CKF',          marker=markers[0], markevery=5000, color='red', markersize = markersize)
##            ax2.plot(ydata[0:t, 2], label=r'$N_{B,a}$ - Ground Truth', marker=markers[0], linewidth=linewidth, markevery=5000, color='lightblue', markersize = markersize)
...
##            ax2.set_ylabel(r'$\phi_B$, $N_{B,a} [uV]$')
##            # plt.xticks(np.arange(0,220/step_size,20/step_size),np.arange(0,220,20))
##            ax.set_xlabel('steps')
##            # ax.set_ylim([-50, 50])
...
##            plt.savefig('Figures/{}_model_err_{}_p_{}_q_{}.png'.format(simulation_date,
##                                                                "{:.7f}".format(err), 
##                                                                str(np.diag(P0.numpy())[-s_dim:]), 
##                                                                str(np.diag(Q0.numpy())[-s_dim:])), bbox_inches='tight', dpi=500) 
##            plt.show()
        # break
            #%%
