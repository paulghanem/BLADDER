...
#     args = parser.parse_args()
    ...
    n_points = 20000
    t=np.linspace(0,300*60,n_points)
    ...
    NBa0 = 0.01; #no afferent signal #no afferent signal
    ...
    #sol=odeint(lut.forward_lsoda,ODE0,t, atol=1e-13,rtol=[1e-6,1e-10,1e-6],mxstep=5000)
...
##
##        if mode=='train':
##            yi_cbf, ei_cbf = nncbf.update(ydata[t],t1, u=xi)
##        elif mode== 'test':
...
#     states = np.zeros((len(t)+1,3), dtype = np.float64)
#     states[0,0]=lut.states['Bladder']['Bladder Volume']
#     states[0,1]=lut.states['Bladder']['Bladder Tension']
#     states[0,2]=lut.states['Bladder']['Bladder Afferent']
# ##    
    ...
    for i, time in enumerate(t):
         ...
         if mode=='train':
             yi_cbf, ei_cbf = nncbf.update(ydata[i],t1, u=xi)
         elif mode== 'test':
             yi_cbf, ei_cbf = nncbf.test_forward(ydata[i],  u=xi)
         else:
             print("mode not found in options")
         
         #sol=odeint(lut.forward_lsoda,states[i,:],t1, atol=1e-13,rtol=[1e-6,1e-10,1e-6],mxstep=5000)
         #states[i+1,:]=sol[1,:]
         
    

    #Plots
    plot_vs_time("VB",   lut.integrator.t[-states.shape[0]:],states[0:len(states)-1,:], bladder.constants)
    plot_vs_time("phiB", lut.integrator.t[-states.shape[0]:],states[0:len(states)-1,:], bladder.constants)
    plot_vs_time("NBas", lut.integrator.t[-states.shape[0]:],states[0:len(states)-1,:], bladder.constants)
